...
リポジトリ定義は `scripts/config/special-repo-definitions.json` の `organization` 配列で管理します。
...
| リポジトリ定義読み込み | `scripts/config/special-repo-definitions.json` の `organization` 配列を読み込み | `jq` |
