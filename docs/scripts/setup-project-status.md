...
| カラム名 | カラー | 説明 | 用途 |
|---------|--------|------|------|
| Backlog | GREY | バックログ | 優先度未確定・いつかやるタスク |
...
| カラム更新 | `singleSelectOptions` に Backlog（GREY）・Todo（BLUE）・In Progress（YELLOW）・In Review（ORANGE）・Done（GREEN）を指定して一括更新 | `gh api graphql` — `updateProjectV2Field` mutation |
