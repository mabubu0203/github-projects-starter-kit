...
| Setup Status Columns | Project の Status カラムをテンプレートで設定する | `workflow_dispatch`（手動実行） |
...
### Status カラムをテンプレートで設定する

1. `Actions` タブを開く
2. `Setup Status Columns` を選択
3. `Run workflow` をクリック
4. パラメータを入力して実行
...
| `template_pattern` | テンプレートパターン | ✅ | `simple`（デフォルト） |
...
| `sprint` | Sprint Backlog → In Progress → In Review → Done → Blocked |
...
.github/workflows/
  ...
  ├── setup-status-columns.yml    # ステータスカラム設定ワークフロー
