...
| Issue/PR をまとめて取り込みたい | [⑥ Issue/PR 一括紐付け](workflows/06-add-items-to-project.md) | Project に Repository の Issue/PR を一括追加 |

### 📈 分析・レポートする

| やりたいこと | Workflow / スクリプト | 説明 |
|-------------|----------------------|------|
| Project の内容を一覧で出したい | [⑦ 統合 Project 分析](workflows/07-analyze-project.md) | Project の Issue/PR 一覧をエクスポート（`report_types: export`） |
| 滞留タスクを検知したい | [⑦ 統合 Project 分析](workflows/07-analyze-project.md) | 指定日数以上動きのないアイテムを自動検出（`report_types: stale`） |
| サマリーレポートを出したい | [⑦ 統合 Project 分析](workflows/07-analyze-project.md) | Status 別・担当者別の集計レポートを生成（`report_types: summary`） |
| 工数・ベロシティを把握したい | [⑦ 統合 Project 分析](workflows/07-analyze-project.md) | 工数集計・ベロシティレポートを生成（`report_types: effort,velocity`） |
